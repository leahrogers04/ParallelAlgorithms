...
void drawPicture()
{
	...
	cudaMemPrefetchAsync(P, N*sizeof(float3), cudaCpuDeviceId); //prefetching positions to host. cudaCpuDeviceId is int - 1 and the device id representing the CPU
	cudaDeviceSynchronize(); //wait for prefetch to finish
...
void setup()
{
	...
	for(int i = 0; i < NumberOfGpus; i++)
    	{
			// cudaMemPrefetchAsync(P, N *sizeof(float3), 0)
			// cudaMemPrefetchAsync(V, N *sizeof(float3), 0)
			cudaMemPrefetchAsync(P, NPerGPU *sizeof(float3), i);
			cudaMemPrefetchAsync(V, NPerGPU *sizeof(float3), i);
			cudaMemPrefetchAsync(F, NPerGPU *sizeof(float3), i);
			cudaMemPrefetchAsync(M, NPerGPU *sizeof(float), i);
