...
void cleanUp();
void pickTheBetterGPU();
void checkGridAndBlockDimensions(int device);
...
void setUpDevices()
{
	...
	{
		printf("you didnt use a power of 2 for your block size stupid doodoo head buttface \n");
		printf("you suck!!!!!!!!!! Be smarter!!!!\n");
		exit(0);
	}
...
void pickTheBetterGPU()
{
	...
	for (int i=0; i < count; i++) 
	{
		...
		if (computeCapability > bestComputeCapability)
		{
			bestGPU = i;
	...
	printf("Selected GPU %d with compute capability %d.%d\n", bestGPU, bestComputeCapability/ 10, bestComputeCapability %10);
}
...
void checkGridAndBlockDimensions(int device)
{
    ...
    if (BlockSize.x > prop.maxThreadsPerBlock) {
        printf("Block size exceeds the maximum number of threads per block for this device.\n");
        exit(0);
    }

    if (GridSize.x > prop.maxGridSize[0]) {
        printf("Grid size exceeds the maximum number of blocks per grid for this device.\n");
        exit(0);
    }

    printf("Grid and block dimensions are within the limits of the device.\n");
}
...
{	
	...
	// Device "GPU" Memory
	cudaMalloc(&A_GPU,BLOCK_SIZE*GridSize.x*sizeof(float));
	cudaErrorCheck(__FILE__, __LINE__);
	cudaMalloc(&B_GPU,BLOCK_SIZE*GridSize.x*sizeof(float));
	cudaErrorCheck(__FILE__, __LINE__);
...
int main()
{
	...
	cudaMemset(A_GPU, 0,BLOCK_SIZE*GridSize.x*sizeof(float));
	cudaMemset(B_GPU, 0,BLOCK_SIZE*GridSize.x*sizeof(float));
