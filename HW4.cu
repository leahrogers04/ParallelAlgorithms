...
void addVectorsCPU(float *a, float *b, float *c, int n)
{
	...
	{ 
		c[id] = sqrt(cos(a[id])*cos(a[id]) + sin(a[id])*sin(a[id]) + a[id]*a[id] - 1.0) + sqrt(cos(b[id])*cos(b[id])  + sin(b[id])*sin(b[id]) + b[id]*b[id] - 1.0);
		//c[id] = a[id] + b[id];
	}
}
...
__global__ void addVectorsGPU(float *a, float *b, float *c, int n)
{
	...
	if(id < n) // Making sure we are not working on memory we do not own.
	{
		...
		//for(int id = 0; id < n; id++)
		{ 
			c[i] = sqrt(cos(a[i])*cos(a[i]) + sin(a[i])*sin(a[i]) + a[i]*a[i] - 1.0) + sqrt(cos(b[i])*cos(b[i])  + sin(b[i])*sin(b[i]) + b[i]*b[i] - 1.0);
		}
