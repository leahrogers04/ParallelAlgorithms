...
__global__ void fillHistogramGPU(float *randomNumbers, int *hist)
{
	...
	{
		float binSize = MAX_RANDOM_NUMBER/(float)NUMBER_OF_BINS;
		float randomValue = randomNumbers[id];

		int binNumber = min((int)(randomValue/binSize), NUMBER_OF_BINS - 1);
		atomicAdd(&sharedHist[binNumber], 1);
	}
	__syncthreads();
